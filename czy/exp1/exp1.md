...
**未决**：本实验同时改动了 terrain（plane→trimesh）与随机化，尚未二分归因（下一步可选：`PLAY_MATCH_TRAIN=1` 但强制 plane，或 plane + 仅补踝/髋-roll armature=0.02）。真机是特定一次动力学抽取，与训练域"平均"不同，部署前应在辨识名义值上单点验证。

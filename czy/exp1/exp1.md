...
### 13.2 证据缺口（决定本轮第一步不能直接开训）

- **正规训练里那 10% 提前终止（ep_len 2168/2400）到底是什么原因**：接触力越界？姿态 0.8rad？高度 0.45？斜坡/离散地形？高 |wz| 段？——**从未分解过**。
- 现有回放**只覆盖直行 0.4/0.6**：训练分布含 vy±0.4、wz±0.6、vx 最到 1.5、地形 level 0~5（flat/rough/slope/discrete）、推力扰动（play 里全无）。
- 单 env（env0）样本量太小（腾空事件三代分别只有 16/11/20 次）→ 结论需多 env 复核。

### 13.3 方案

**Step 1（先做，零训练额度，本地 ~15min）——训练域多 env 稳定性诊断**
- 512 env × 3 episodes（训练配置原样：trimesh + 随机化 + 噪声 + gait 调度 + 推力），policy 用均值；记录每 episode：
  ① **终止原因分解**（接触力 / |roll|>0.8 / |pitch|>0.8 / h<0.45 / timeout）；
  ② **条件切片**：地形类型与 level、指令桶（|vx|、|vy|、|wz|）、gait 段；
  ③ **余量分布**：h min、|roll|/|pitch| max 的 p50/p95；
  ④ **落地相**：双支撑/单支撑/腾空（≥50ms）占比及其与终止的相关性。
- 产出"稳定性预算表"→ 确定 Step 2 打哪个靶（这才是 exp1.13 的立项依据）。

**Step 2（按 Step 1 数据二选一或多选，预注册）**

| 触发条件 | 对应修改 |
| --- | --- |
| 双支撑 <15% / 腾空 ≥50ms 事件普遍存在，且与终止相关 | **主改**：`foot_place` 步幅系数 0.75→0.65（缩短步幅直接增大双支撑窗）+ 新增"双支撑窗内双脚接触"奖励项（|sin 2πφ|<0.1 时按接触给分） |
| 终止集中在高 |wz| 或 |vy| 段 | 转向/侧移能力单独补锚（现有 walk_turn 参考段利用率低） |
| 终止集中在 rough/discrete 地形 | 地形配比再平衡或降 max_init_terrain_level（当前 curriculum=False 固定 0~5） |
| 终止多为 h<0.45 触线而非真摔 | 放宽 base_height_cutoff → 0.42（exp1.12 判据本属误判产物，训练域 ep_len 未见其收益） |

**Step 3（需时再上云端）**：底模 model_37997，8000 轮（29998 语义同往例），[8] 账号，预算 ~¥30；判据：双支撑 ≥15%、腾空(≥50ms)=0、单支撑 ≤80%、ep_len 不降、style ≥0.10、名义模式航向漂移 ≤0.06 rad/s。止损：style<0.10 或 ep_len 掉 10% → 撤最新改动。

**本轮不做**：速度类改动（0.4 档 1.42~1.6x 超速）——按用户指令先搁置，且其口径尚未定标（§21h）。

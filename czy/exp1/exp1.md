...
**机理（OOD 动力学）**：
1. **关节 armature 覆盖不全**：`fixed_armature` 只列 hip_pitch/hip_yaw/knee/上半身，**踝 pitch/roll、髋 roll 未列**→回退 URDF 默认（URDF 无 armature 字段 → 0）；而训练范围 ankle `[0.003,0.04]`、hip_roll `[0.0001,0.05]`，**0 落在训练分布之外**→踝等效惯量偏低、PD 带宽过高→姿态（尤其 pitch 前冲）发散。play 注释"回放保持 0"正是此处失误。
2. `randomize_joint_friction`（训练 `[0.01,1.15]`）关闭 → 关节摩擦回退 0；
3. 刚体摩擦/质量/COM/gains/torque/motor_offset/噪声 全关 → 策略被放到训练分布的"一个手工点"而非其分布中心。
